      ...
      <CardContent className='flex flex-row items-center'>
        ...
        <div className='overflow-hidden max-w-[196px] flex-1 h-full'>
          {isLoading ? (
            ...
            <CardTitle className='text-sm max-h-full line-clamp-2 font-semibold'>{data?.title ?? 'Unknown'}</CardTitle>
          )}
          {isLoading ? (
            <Skeleton className='w-full rounded-lg' />
          ) : (
            <CardDescription>via {data?.provider_name ?? 'Unknown'}</CardDescription>
          )}
        </div>
      </CardContent>
    </Card>
